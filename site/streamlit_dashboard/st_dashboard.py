...
def box_plot_all(df, num_client, cols, decision):
    ...
    sns.boxplot(data=df_melt, x='variables', y='values',  hue='TARGET', linewidth=1, showfliers=False,
    ...
    print(f"df: {df}")
    # print(f"df.loc[str100002][cols]: {df.loc[100002][cols]}")
...
st.write("Dashboard réalisée par Baptiste LEDESERT - Parcours Data Science - projet 7")
...
feat_imp_global = st.sidebar.checkbox("Facteurs d'influence globaux")
...
if num != '<Select>':
    ...
    if influence:
        ...
        print(f"type(feat_names: {type(feat_names)}")
    ...
    if comp_infos_tous_clients:
        ...
        if comp_clients == "Groupe clients":
            ...
            st.write("Données pour un échantillon de 5 clients")
            st.dataframe(df_infos_scores.sample(5))
            ...
            if st.checkbox('Analyse bivariée des caractéristiques à sélectionner des clients'):
                to_select = ['NAME_INCOME_TYPE', 'NAME_EDUCATION_TYPE', 'AMT_INCOME_TOTAL', 'AMT_CREDIT', 'AMT_ANNUITY',
                             'AMT_GOODS_PRICE', 'years_employed', 'Age', 'NAME_CONTRACT_TYPE']
        ...
        if comp_clients == "Clients similaires":
            ...
            if st.checkbox('Analyse bivariée des caractéristiques à sélectionner des clients similaires'):
                to_select_sim = ['AMT_INCOME_TOTAL', 'AMT_CREDIT', 'AMT_ANNUITY',
                                  'AMT_GOODS_PRICE','years_employed', 'Age']
                ...
                if (x != '<Select>') & (y != '<Select>'):
                    ...
                    df_voisins["Decision"] = ['Granted' if elt == 0 else 'Refused' for elt in df_voisins["TARGET"]]
